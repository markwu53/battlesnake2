...
class Snake:
    ...
    def dict(self):
        return {k: self.__dict__[k] for k in ["name", "health", "length", "body", 
                                              #comment out ID, I don't use it for now
                                              #"id", 
                                              ]}
    def copy(self):
...
def main(game_state, log=True, log_db=False):
    ...
    def shorter_goto_territory_border(moves):
        ...
        territory_border = [a for a in territory_border for p in adj_cells(a) if p in g.me.territory and p not in territory_border]
        ...
        target_terminal = prefer_by_score(lambda a: sum(distance_to_border(a)))(target_terminal)
    ...
    def avoid_short_vulnerable_move(moves):
        ...
        killers = [snake for snake in g.others if snake.length > g.me.length]
        if len(killers) == 0: return
        danger_length = min([path_distance_pq(g.me.head, snake.head) for snake in killers])
        single_move_length = danger_length//2 -1
        vulnerable_move = [a for a in moves if single_move_n(single_move_length, a)
                           and any([path_distance_pq(killer.head, a) <= danger_length for killer in killers])
                           ]
    ...
    def type_2_collision(moves):
        ...
        if len(g.others) > 1 and path_distance_pq(avoid, g.me.tail) <= 2:
            avoid_to_tail = shortest_path_move(avoid, g.me.tail)
            if not any([distance_pq(a, killer.head) < distance_pq(avoid, killer.head) for a in avoid_to_tail]):
                #killer is not in the middle between avoid and tail
    ...
    def avoid_next_step_confinement(moves):
        ...
        for a in moves:
            ...
            for b in killer.allowed_moves:
                ...
                if b == a: continue
                if distance_vector_abs(b, a) == (1,1): continue
                ...
                occupied = complement(me2.territory)
    ...
    def has_wayout_on_others2(aset, a):
        ...
        for snake in g.others:
            adjacent_indexes = [i
                    for i,c in enumerate(snake.body) if c != snake.tail and c != snake.head and c not in cut_set
                    for p in adj_cells(c) if p in aset and p != a
                    ]
            if len(adjacent_indexes) == 0: continue
            max_index = max(adjacent_indexes)
    ...
    def attack_vulnerables_on_point(moves):
        for snake in g.vulnerables:
            ...
            if len(attack_move) != 0:
                g.decision_path.append(f"attack vulnerables on point exact {snake.name}")
                return attack_move
    ...
    def attack_vulnerables_on_point_neg2(moves):
        for snake in g.vulnerables:
            ...
            if len(attack_move) == 1:
                ...
                if len(attack_move) != 0:
                    g.decision_path.append(f"attack vulnerables on point negative 2 {snake.name}")
                    return attack_move
            else:
                #len(attack_move) == 2
    ...
    def attack_vulnerables_on_point_neg_more(moves):
        for snake in g.vulnerables:
            ...
            if len(attack_move) != 0:
                g.decision_path.append(f"attack vulnerables on point negative more {snake.name}")
                return attack_move
    ...
    def attack_vulnerables_on_point_positive(moves):
        for snake in g.vulnerables:
            ...
            if excess in [2,4,6]: 
                ...
                attack_move = [a for a in moves if a in attack_move]
                if len(attack_move) != 0:
                    g.decision_path.append(f"attack vulnerables on point positive 8 {snake.name}")
                    return attack_move
    ...
    def attack_vulnerables_suppress(moves):
        for snake in g.vulnerables:
            ...
            attack_move = [a for a in moves if a in attack_move]
            if len(attack_move) != 0:
                g.decision_path.append(f"attack vulnerables suppress exact {snake.name}")
                return attack_move
    ...
    def attack_vulnerables_suppress_neg2(moves):
        for snake in g.vulnerables:
            ...
            if len(attack_move) == 1:
                ...
                attack_move = [a for a in moves if distance_vector_abs(a, attack_move) == (1,1) and any([path_connected(a, p) for p in attack_move])]
                if len(attack_move) != 0:
                    g.decision_path.append(f"attack vulnerables suppress negative 2 {snake.name}")
                    return attack_move
            else:
                #len(attack_move) == 2
                attack_move = [a for a in moves if a not in attack_move and any([path_connected(a, p) for p in attack_move])]
    ...
    def attack_vulnerables_suppress_neg_more(moves):
        for snake in g.vulnerables:
            ...
            attack_move = [a for a in moves if a not in attack_move and any([path_connected(a, p) for p in attack_move])]
            if len(attack_move) != 0:
                g.decision_path.append(f"attack vulnerables suppress negative more {snake.name}")
                return attack_move
    ...
    def avoid_next_step_immediate_kill(moves):
        snake = [snake for snake in g.others if snake.length > g.me.length and distance_vector_abs(g.me.head, snake.head) == (1,1)]
        if len(snake) != 1: return
        snake = take_first(snake)
        collision = [a for a in moves if a in snake.allowed_moves]
        if len(collision) == 2: return
...
if __name__ == "__main__":
    ...
    log = {'id': '51b87d96-5ad1-4b96-8b30-158eefe46d0b', 'turn': 253, 'me': {'name': 'mark_snake', 'health': 64, 'length': 18, 'body': [(3, 8), (3, 7), (3, 6), (3, 5), (3, 4), (4, 4), (4, 5), (5, 5), (5, 4), (5, 3), (4, 3), (3, 3), (2, 3), (1, 3), (1, 4), (1, 5), (1, 6), (1, 7)]}, 'others': [{'name': 'Kakemonsteret-v2', 'health': 91, 'length': 24, 'body': [(4, 9), (5, 9), (5, 8), (6, 8), (7, 8), (7, 7), (7, 6), (7, 5), (7, 4), (7, 3), (8, 3), (8, 4), (8, 5), (8, 6), (8, 7), (9, 7), (9, 6), (9, 5), (9, 4), (9, 3), (10, 3), (10, 4), (10, 5), (10, 6)]}, {'name': 'Red Yarn', 'health': 88, 'length': 16, 'body': [(7, 0), (8, 0), (9, 0), (10, 0), (10, 1), (9, 1), (8, 1), (7, 1), (6, 1), (5, 1), (4, 1), (3, 1), (2, 1), (2, 2), (3, 2), (4, 2)]}], 'food': [(0, 2)], 'module': 'decision_flow - github', 'decision_path': ['1vn', "vulnerable snakes: [('Red Yarn', 4, (3, 0))]", 'collision type 2 bad tail take risk'], 'next_coord': (4, 8), 'next_move': 'right', 'time': '0.013s'}
    log = {'id': 'af9101ae-69a4-47ef-87fe-9c24567d7187', 'turn': 224, 'me': {'name': 'mark_snake', 'health': 75, 'length': 19, 'body': [(6, 4), (7, 4), (8, 4), (9, 4), (9, 3), (8, 3), (7, 3), (6, 3), (5, 3), (4, 3), (3, 3), (2, 3), (2, 4), (2, 5), (1, 5), (1, 4), (0, 4), (0, 3), (0, 2)]}, 'others': [{'name': '@~~~~@', 'health': 56, 'length': 17, 'body': [(5, 5), (5, 6), (4, 6), (4, 7), (3, 7), (3, 8), (3, 9), (3, 10), (4, 10), (5, 10), (5, 9), (4, 9), (4, 8), (5, 8), (6, 8), (7, 8), (8, 8)]}], 'food': [(9, 0), (9, 7)], 'module': 'decision_flow - github', 'decision_path': ['1v1', 'preliminary cut kill target: @~~~~@', 'try collision cut kill (5, 4)'], 'next_coord': (5, 4), 'next_move': 'left', 'time': '0.008s'}
    log = {'id': '7b5b1467-38c8-466e-b573-627c9303279d', 'turn': 374, 'me': {'name': 'mark_snake', 'health': 88, 'length': 23, 'body': [(7, 7), (7, 6), (7, 5), (7, 4), (7, 3), (7, 2), (7, 1), (8, 1), (9, 1), (10, 1), (10, 2), (10, 3), (9, 3), (9, 4), (10, 4), (10, 5), (10, 6), (10, 7), (10, 8), (10, 9), (10, 10), (9, 10), (9, 9)]}, 'others': [{'name': 'SmartyRat', 'health': 83, 'length': 17, 'body': [(5, 7), (4, 7), (4, 6), (4, 5), (4, 4), (4, 3), (4, 2), (5, 2), (5, 1), (5, 0), (6, 0), (6, 1), (6, 2), (6, 3), (5, 3), (5, 4), (5, 5)]}], 'food': [(0, 5), (2, 7), (0, 10), (0, 6), (1, 7), (1, 5), (1, 1), (0, 1), (1, 9), (3, 7), (4, 10), (3, 0), (2, 0)], 'module': 'decision_flow - github', 'decision_path': ['1v1', '1v1 longer push', 'longer confront push'], 'next_coord': (6, 7), 'next_move': 'left', 'time': '0.021s'}
    log = {'id': '808f089d-704d-4215-80c9-7fabafc5835a', 'turn': 348, 'me': {'name': 'mark_snake', 'health': 95, 'length': 29, 'body': [(3, 7), (3, 6), (3, 5), (2, 5), (2, 4), (2, 3), (1, 3), (1, 4), (1, 5), (1, 6), (1, 7), (1, 8), (0, 8), (0, 7), (0, 6), (0, 5), (0, 4), (0, 3), (0, 2), (0, 1), (1, 1), (2, 1), (3, 1), (4, 1), (5, 1), (5, 2), (5, 3), (4, 3), (4, 2)]}, 'others': [{'name': 'Copy of snake2_v3_FINAL_final(1)', 'health': 100, 'length': 33, 'body': [(6, 8), (6, 7), (6, 6), (5, 6), (5, 5), (5, 4), (6, 4), (6, 3), (6, 2), (6, 1), (6, 0), (7, 0), (8, 0), (8, 1), (8, 2), (8, 3), (8, 4), (8, 5), (7, 5), (7, 6), (8, 6), (9, 6), (9, 7), (10, 7), (10, 8), (9, 8), (9, 9), (9, 10), (8, 10), (7, 10), (6, 10), (5, 10), (5, 10)]}], 'food': [(1, 9)], 'module': 'decision_flow - github', 'decision_path': ['1v1', 'avoid next step confinement [(2, 7)]', 'avoid collision type 2'], 'next_coord': (3, 8), 'next_move': 'up', 'time': '0.020s'}
