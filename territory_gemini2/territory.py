...
def decision_flow(g: GameTurn, is_pred):
    ...
    def avoid_general_possible_confine(moves):
        ...
        # others = get_relevant_opponents_next_steps(g)
        def confined(a):
            if a not in g.me.territory: return False
            me2 = snake_next_step(g.me, a)
            ng = next_game_turn([me2]+others)
            flood_game_turn(ng)
            ng.set_me(me2)
                ...
                def next_step_wayout():
                    for b in me2.allowed_moves:
                        ...
                        if has_wayout(nng):
                            return True
                    return False
                if next_step_wayout(): return False
            return True

        confined_moves = [a for a in moves if confined(a)]
        if len(confined_moves) == 0: return

        moves = [a for a in moves if a not in confined_moves]
        if len(moves) == 0:
            if not is_pred: g.me.decision_path.append(f"all possible confined")
            return
...
if __name__ == "__main__":
    ...
    log = {'id': 'b2b742e6-21c7-444c-b8d7-ac6c4021ddd3', 'turn': 142, 'me': {'name': 'mark_snake', 'health': 94, 'length': 13, 'body': [(7, 7), (7, 8), (8, 8), (9, 8), (9, 9), (10, 9), (10, 10), (9, 10), (8, 10), (7, 10), (6, 10), (6, 9), (6, 8)], 'id': 'gs_GmgSjwrVgQPDQ3B6wckxmTQ9'}, 'others': [{'name': 'Sandworm', 'health': 69, 'length': 7, 'body': [(10, 6), (10, 5), (9, 5), (8, 5), (7, 5), (6, 5), (6, 4)], 'id': 'gs_8tjMJtHSXGKVkYcdTQrtxBXB'}, {'name': 'Aurora', 'health': 72, 'length': 8, 'body': [(4, 4), (4, 5), (4, 6), (4, 7), (3, 7), (3, 6), (2, 6), (2, 7)], 'id': 'gs_ctPKDVg86xG4797mHCDq9YBP'}, {'name': 'Geriatric Jagwire', 'health': 58, 'length': 12, 'body': [(9, 3), (9, 4), (8, 4), (8, 3), (8, 2), (7, 2), (6, 2), (5, 2), (4, 2), (3, 2), (2, 2), (1, 2)], 'id': 'gs_SGW6dS7YmPgdVht6fqJjK4vS'}], 'food': [(9, 1), (9, 2)], 'module': 'territory', 'decision_path': ['border analysis move go (8, 6)'], 'next_coord': (7, 6), 'next_move': 'down', 'time': '0.030s'}
