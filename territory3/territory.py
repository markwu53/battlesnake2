...
def main(game_state, log=True):
    ...
    def decision_flow(moves):
        return seq_next([ id
            ...
            , prefer(in_territory)
            , cond(g.me.length >= 8)(target_confront)
            , goto_center
            , cond(g.me.length < 8)(prefer_not(on_border))
    ...
    def shortest_path_moves(p):
        moves = [a for a in g.me.allowed_moves if tree_distance(a, p) >= 0]
        return moves
    ...
    def killer_confront(moves):
        ...
        snake = take_first(killers)

        confront_moves = [a for a in moves if a in g.me.to_snake_border[snake.head]]
        if len(confront_moves) != 1: return
        if distance_vector_abs(g.me.head, snake.head) not in [(0,4), (4,0)]: return
        ...
        killer_move = [a for a in snake.allowed_moves if distance_pq(a, confront_move) == 2]
        if len(killer_move) != 1: return
        killer_move = take_first(killer_move)
        me2 = snake_next_step(g.me, confront_move)
        snake2 = snake_next_step(snake, killer_move)
        ...
        if enough_room: return
        moves = [a for a in moves if a not in confront_moves]
        if len(moves) != 0:
            g.decision_path.append(f"confront not enough room avoid {confront_moves}")
            return moves

    def target_confront(moves):
        targets = [snake for snake in g.others if True
                   and g.me.length > snake.length
                   and distance_pq(snake.head, g.me.head) == 4
                   and len(g.me.to_snake_border[snake.head]) != 0 ]
        if len(targets) == 0: return

        snake = take_first(take_first_group(key=lambda s: len(s.territory))(targets))

        moves = [a for a in moves if True
                 and a in g.me.territory 
                 and g.me.territory_connection_number[a] != 1 
                 and distance_pq(a, snake.head) == 3 ]
        if len(moves) != 0:
            g.decision_path.append(f"push target {snake.name} {moves}")
            return moves

    def goto_center(moves):
        if len(g.me.territory) == 0: return
        center = take_first_group(lambda p: -sum_xy(distance_to_border(p)))(g.me.territory)
        center = take_first(center)
        center_moves = shortest_path_moves(center)
        moves = [a for a in moves if a in center_moves ]
        if len(moves) != 0:
            g.decision_path.append(f"center move {center}")
            return moves
...
if __name__ == "__main__":
    ...
    log = {'id': '57f49340-48a0-4d68-b552-425731d35d23', 'turn': 197, 'nalive': 3, 'snakes': [{'name': 'mark_snake_test RED', 'health': 87, 'length': 23, 'alive': True, 'delay': 1, 'body': [(4, 7), (5, 7), (6, 7), (7, 7), (8, 7), (8, 6), (8, 5), (7, 5), (6, 5), (5, 5), (5, 4), (6, 4), (7, 4), (7, 3), (6, 3), (5, 3), (5, 2), (6, 2), (6, 1), (7, 1), (8, 1), (9, 1), (9, 2)]}, {'name': 'mark_snake_test BLUE', 'health': 95, 'length': 19, 'alive': True, 'delay': 0, 'body': [(0, 5), (0, 6), (0, 7), (0, 8), (0, 9), (1, 9), (1, 8), (1, 7), (1, 6), (1, 5), (2, 5), (3, 5), (4, 5), (4, 4), (3, 4), (3, 3), (2, 3), (2, 2), (3, 2)]}, {'name': 'mark_snake_test GREEN', 'health': 85, 'length': 20, 'alive': True, 'delay': 13, 'body': [(3, 8), (4, 8), (5, 8), (6, 8), (7, 8), (8, 8), (9, 8), (9, 9), (9, 10), (8, 10), (7, 10), (6, 10), (5, 10), (4, 10), (3, 10), (2, 10), (2, 9), (2, 8), (2, 7), (2, 6)]}, {'name': 'mark_snake_test YELLOW', 'health': 87, 'length': 6, 'alive': False, 'delay': 0, 'body': [(9, 10), (10, 10), (10, 9), (10, 8), (10, 7), (10, 6)]}], 'food': [(10, 8), (9, 6), (10, 2), (3, 9), (10, 10), (8, 3), (1, 3)]}
