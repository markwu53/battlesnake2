...
def avoid_border_type_1_collision(moves):
    ...
    snakes = [snake for snake in g.others if distance_pq(snake.head, g.me.head) == 4 and distance_pq(snake.head, g.me.head) == 4]
    if len(snakes) != 1: return
    snake = take_first(snakes)
    if snake.length <= g.me.length: return
    danger_move = [a for a in moves if on_border(a) for b in snake.allowed_moves if on_border(a,b) in [(0,2), (2,0)] and not on_border(b)]
...
def avoid_next_step_suppressed_2(moves):
    ...
    if len(snakes) != 1: return
    snake = take_first(snakes)
    suppressed_move = [a for a in moves for b in snake.allowed_moves if distance_vector_abs(a,b) in [(0,2), (2,0)] and not distance_vector_abs(b)]
...
def avoid_cornered_bordered(moves):
    ...
    if sum(distance_to_border(g.me.head)) <= 1:
        ...
        if distance_pq(g.me.head, g.other.head) <= 6:
            if path_distance_pq(g.me.head, g.other.head) == path_distance_pq(g.me.head, g.other.head):
                g.decision_path.append("avoid cornered bordered")
                #return prefer_by_rank(lambda a: min(prefer_by_rank(a, g.other.head)))(moves)
                return prefer_by_rank(lambda a: prefer_by_rank(a, g.other.head))(moves)
...
def prefer_away_border(moves):
    ...
    return prefer_by_score(lambda a: min(*prefer_by_score(a), 2))(moves)
...
def avoid_two_snake_trap_config_24(moves):
    ...
    if len(snakes) != 1: return
    snake = take_first(snakes)
    collision = take_first([a for a in g.me.allowed_moves if take_first(a, snake.head)])
    straight = take_first([a for a in g.me.allowed_moves if take_first(a)])
...
def avoid_two_snake_trap_config_10(moves):
    ...
    avoid = take_first([a for a in type_2_collision if take_first(a, single_collision) != (1,1)])
    g.decision_path.append("avoid two-snake trap")
    return [avoid]
...
def avoid_offborder_trap(moves):
    ...
    neck_adj = take_first([a for a in take_first(g.me.neck) if take_first(a)])
